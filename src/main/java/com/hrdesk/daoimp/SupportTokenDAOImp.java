...
/**
 * JDBC implementation of SupportTokenDAO.
 * Maps to the 'support_tokens' table.
 * DB columns: token_id, emp_id, title, description, category, priority, status, assigned_to, created_at, resolved_at
 */
public class SupportTokenDAOImp implements SupportTokenDAO {

    // Helper to map ResultSet row → SupportTokenDTO
    ...
    public boolean updateToken(SupportTokenDTO token) {
        ...
        try (Connection con = Connector.getConnection();
            ...
            if (token.getAssignedTo() > 0) ps.setInt(6, token.getAssignedTo());
