 ...
 * DB columns: att_id, emp_id, date, status, check_in, check_out
 */
public class AttendanceDAOImp implements AttendanceDAO {

    // Helper to map ResultSet row → AttendanceDTO
    private AttendanceDTO mapRow(ResultSet rs) throws SQLException {
        ...
        if (ci != null && co != null) {
            ...
            if (diff > 0) att.setWorkingHours(diff / 3600000.0);
        }
        return att;
    }
    ...
    public boolean markAttendance(AttendanceDTO att) {
        ...
        try (Connection con = Connector.getConnection();
            ...
            ps.setDate(2, att.getAttendanceDate() != null
                    ? new java.sql.Date(att.getAttendanceDate().getTime()) : null);
