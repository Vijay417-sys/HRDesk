<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ page import="com.hrdesk.dto.User, com.hrdesk.dto.PayrollDTO, java.util.List" %>
<%
    User user = (User) session.getAttribute("user");
    if (user == null || !"ADMIN".equals(user.getRole())) {
        response.sendRedirect(request.getContextPath() + "/login.jsp"); return;
    }
    List<PayrollDTO> payrolls = (List<PayrollDTO>) request.getAttribute("payrollList");
%>
<%@ include file="../includes/header.jsp" %>
<script>document.getElementById('page-title').textContent='Payroll';document.getElementById('page-breadcrumb').textContent='HRDesk / Admin / Payroll';</script>

<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <!-- Generate Payroll Panel -->
    <div class="lg:col-span-1">
        <div class="card p-6 mb-5" style="box-shadow:0 1px 4px rgba(0,0,0,.06);">
            <h3 class="text-sm font-semibold text-gray-900 mb-4">Generate Payroll</h3>
            <form action="<%= request.getContextPath() %>/payroll" method="post" class="space-y-4">
                <input type="hidden" name="action" value="generate">
                <div>
                    <label class="input-label" for="payMonth">Month *</label>
                    <input type="month" id="payMonth" name="payMonth" class="input-field" required>
                </div>
                <div>
                    <label class="input-label" for="deptFilter">Department</label>
                    <select id="deptFilter" name="deptFilter" class="input-field">
                        <option value="">All Departments</option>
                        <option value="1">Engineering</option>
                        <option value="2">Human Resources</option>
                        <option value="3">Finance</option>
                    </select>
                </div>
                <div>
                    <label class="input-label" for="bonus">Bonus (%)</label>
                    <input type="number" id="bonus" name="bonus" class="input-field" placeholder="0" min="0" max="100">
                </div>
                <div>
                    <label class="input-label" for="deduction">Deduction (%)</label>
                    <input type="number" id="deduction" name="deduction" class="input-field" placeholder="0" min="0" max="100">
                </div>
                <button type="submit" class="btn-primary w-full" style="justify-content:center;">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                    Run Payroll
                </button>
            </form>
        </div>

        <!-- Quick Stats -->
        <div class="card p-5" style="box-shadow:0 1px 4px rgba(0,0,0,.06);">
            <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">This Month</h3>
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Employees Paid</span>
                    <span class="text-sm font-semibold text-gray-900">—</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Total Disbursed</span>
                    <span class="text-sm font-semibold text-green-600">₹—</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Pending</span>
                    <span class="text-sm font-semibold text-amber-600">—</span>
                </div>
            </div>
        </div>
    </div>

    <!-- Payroll Records -->
    <div class="lg:col-span-2">
        <div class="card" style="box-shadow:0 1px 4px rgba(0,0,0,.06);">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-gray-900">Payroll Records</h3>
                <button class="btn-secondary" style="padding:.375rem .75rem;font-size:.8125rem;" onclick="alert('Export payroll PDF/CSV — connect to servlet.')">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                    Export
                </button>
            </div>
            <div class="overflow-x-auto">
                <table class="data-table">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Employee</th>
                            <th>Month</th>
                            <th>Base Salary</th>
                            <th>Bonus</th>
                            <th>Deductions</th>
                            <th>Net Pay</th>
                            <th>Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <% if (payrolls != null && !payrolls.isEmpty()) {
                            int sn = 1;
                            for (PayrollDTO p : payrolls) { %>
                        <tr>
                            <td class="text-gray-400 text-xs"><%= sn++ %></td>
                            <td class="font-medium text-gray-900">Emp #<%= p.getEmployeeId() %></td>
                            <td class="text-gray-500"><%= p.getPayMonth() != null ? p.getPayMonth() : "—" %></td>
                            <td class="text-gray-900">₹<%= String.format("%,.0f", p.getBaseSalary()) %></td>
                            <td class="text-green-600">+₹<%= String.format("%,.0f", p.getBonus()) %></td>
                            <td class="text-red-500">-₹<%= String.format("%,.0f", p.getDeductions()) %></td>
                            <td class="font-semibold text-gray-900">₹<%= String.format("%,.0f", p.getNetPay()) %></td>
                            <td><span class="badge <%= "PAID".equals(p.getStatus()) ? "badge-green" : "badge-yellow" %>"><%= p.getStatus() != null ? p.getStatus() : "PENDING" %></span></td>
                        </tr>
                        <% } } else { %>
                        <tr>
                            <td colspan="8" class="py-16 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    <svg class="w-10 h-10 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                    <p class="text-sm text-gray-400">No payroll records yet. Generate payroll to begin.</p>
                                </div>
                            </td>
                        </tr>
                        <% } %>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>

<%@ include file="../includes/footer.jsp" %>