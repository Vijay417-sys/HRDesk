...
<%
    ...
    List<LeaveDTO> leaves = (List<LeaveDTO>) request.getAttribute("leaveList");
%>
        ...
        <table class="data-table" id="leave-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-gray-500 text-center"><%= lv.getTotalDays() %></td>
                    <td class="text-gray-500" style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<%= lv.getReason() %>"><%= lv.getReason() != null ? lv.getReason() : "—" %></td>
                    <td>
                        <span class="badge
                    ...
                    <td>
                        <% if ("PENDING".equals(lv.getStatus()) || lv.getStatus() == null) { %>
                        <div class="flex gap-1.5">
                            <button onclick="approveLeave(<%= lv.getLeaveId() %>)" class="btn-success" style="padding:.25rem .625rem;font-size:.75rem;">Approve</button>
                            <button onclick="rejectLeave(<%= lv.getLeaveId() %>)" class="btn-danger" style="padding:.25rem .625rem;font-size:.75rem;">Reject</button>
                        </div>
                        <% } else { %>
...
function approveLeave(id) {
    if (confirm('Approve this leave request?')) {
        window.location.href = '<%= request.getContextPath() %>/leave?action=approve&id=' + id;
    }
}
function rejectLeave(id) {
    if (confirm('Reject this leave request?')) {
        window.location.href = '<%= request.getContextPath() %>/leave?action=reject&id=' + id;
    }
}
